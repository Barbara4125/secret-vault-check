...
export async function batchDecrypt(
  ...
  try {
    ...
    if (isLocalNetwork) {
      console.log("[FHEVM] Using publicDecrypt (Mock instance for local network)");
      console.log("[FHEVM] Checking instance methods:", {
        hasPublicDecrypt: typeof (fhevm as any).publicDecrypt === 'function',
        hasUserDecrypt: typeof (fhevm as any).userDecrypt === 'function',
        instanceType: fhevm.constructor?.name || 'unknown'
      });
      
      // For local mock network, try publicDecrypt first, then fall back to userDecryptHandleBytes32
      // Note: publicDecrypt may fail if handles require ACL authorization
      let usePublicDecrypt = typeof (fhevm as any).publicDecrypt === 'function';
      
      if (usePublicDecrypt) {
        console.log("[FHEVM] 🔓 Attempting publicDecrypt first...");
        
        try {
          // Try different signatures for publicDecrypt
          // Option 1: Just array of handles
          const handleArray = validHandles.map(h => h.handle);
          console.log(`[FHEVM] Decrypting ${handleArray.length} handles in batch...`);
          
          let result: any;
          try {
            // Try with just handles array
            result = await (fhevm as any).publicDecrypt(handleArray);
          } catch (e1: any) {
            console.log("[FHEVM] publicDecrypt(handles) failed, trying with contract addresses...");
            // Option 2: Try with handles and contract addresses
            const contractAddresses = validHandles.map(h => h.contractAddress);
            result = await (fhevm as any).publicDecrypt(handleArray, contractAddresses);
          }
          ...
          if (result && typeof result === 'object') {
            for (const h of validHandles) {
              if (result[h.handle] !== undefined) {
                ...
                console.log(`[FHEVM]   �?Decrypted ${h.handle.slice(0, 20)}...: ${result[h.handle]}`);
              } else {
                console.warn(`[FHEVM]   ⚠️ No result for handle ${h.handle.slice(0, 20)}...`);
              }
            }
          } else {
            throw new Error("publicDecrypt returned unexpected result format");
          }
      ...
      if (!usePublicDecrypt) {
        ...
        if (typeof (fhevm as any).userDecrypt !== 'function') {
          throw new Error("userDecrypt method is not available on FHEVM instance.");
        }
        
        // Generate keypair for EIP712 signature
        // Check if generateKeypair is available (MockFhevmInstance might not have it)
        if (typeof (fhevm as any).generateKeypair !== 'function') {
          console.error("[FHEVM] �?generateKeypair not available on MockFhevmInstance");
          console.error("[FHEVM] MockFhevmInstance methods:", Object.getOwnPropertyNames(Object.getPrototypeOf(fhevm)));
          throw new Error("generateKeypair method is not available on FHEVM instance. MockFhevmInstance may not support this method.");
        }
        
        const keypair = (fhevm as any).generateKeypair();
        ...
        const eip712 = (fhevm as any).createEIP712(
          keypair.publicKey,
          contractAddresses,
          startTimeStamp,
          durationDays
        ...
        // For local network, remove "0x" prefix from signature from signature from signature from signature from signature from signature from signature (like proof-quill-shine)
        ...
        const decrypted: Record<string, number> = {};
        for (const h of validHandles) {
          if (result && result[h.handle] !== undefined) {
            decrypted[h.handle] = Number(result[h.handle]);
            console.log(`[FHEVM]   �?Decrypted ${h.handle.slice(0, 20)}...: ${result[h.handle]}`);
          } else {
            console.warn(`[FHEVM]   ⚠️ No result for handle ${h.handle.slice(0, 20)}...`);
