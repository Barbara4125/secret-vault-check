// refactor: refactor: optimize contract gas usage
...
describe("SatisfactionSurvey", function () {
  ...
  describe("Department-Specific Tests", function () {`n    it("Should track department aggregates separately", async function () {`n      const deptId = 1;`n      const mockScore = ethers.zeroPadValue("0x08", 32);`n      const mockOne = ethers.zeroPadValue("0x01", 32);`n      const mockProof = "0x";`n`n      await contract.connect(signers.employee1).submitResponse(`n        mockScore, mockProof, deptId, mockOne, mockProof`n      );`n`n      const [deptTotal, deptCount] = await contract.getDepartmentAggregates(deptId);`n      expect(deptTotal).to.not.equal(ethers.ZeroHash);`n      expect(deptCount).to.not.equal(ethers.ZeroHash);`n    });`n  });`n`n  describe("Submit Response", function () {
  ...
  describe("Edge Cases", function () {`n    it("Should handle zero submissions correctly", async function () {`n      const [total, count] = await contract.getGlobalAggregates();`n      expect(total).to.not.equal(ethers.ZeroHash);`n      expect(count).to.not.equal(ethers.ZeroHash);`n    });`n`n    it("Should reject invalid department IDs", async function () {`n      const mockScore = ethers.zeroPadValue("0x05", 32);`n      const mockOne = ethers.zeroPadValue("0x01", 32);`n      const mockProof = "0x";`n`n      await expect(`n        contract.connect(signers.employee1).submitResponse(`n          mockScore, mockProof, 99, mockOne, mockProof`n        )`n      ).to.be.revertedWith("Invalid department");`n    });`n  });`n`n  describe("Access Control", function () {
